...
POST_FIELDS = ["post_id", "user_id", "post_type", "post_title", "post_author", "timeline_icon", "timestamp", "post_image", "post_content", "post_tags", "post_description"]
...
def getAllPosts(content):
	...
	with closing(content.cursor()) as cursor:
		currentQuery = "SELECT post_id, user_id, post_type, post_title, post_author, timeline_icon, timestamp, post_image, post_content, post_tags, post_description FROM posts ORDER BY timestamp DESC;"
...
def getPost(content, post_id):
	...
	with closing(content.cursor()) as cursor:
		currentQuery = "SELECT post_id, user_id, post_type, post_title, post_author, timeline_icon, timestamp, post_image, post_content, post_tags, post_description FROM posts WHERE post_id=%s;"
...
def savePost(content, postData):
	with closing(content.cursor()) as cursor:
		currentQuery ="UPDATE posts SET post_type=%s, post_title=%s, post_author=%s, timeline_icon=%s, post_image=%s, post_content=%s, post_tags=%s, post_description=%s WHERE post_id=%s;"
		
		try:
			cursor.execute(currentQuery, (postData["post_type"], postData["post_title"], postData["post_author"],postData["timeline_icon"], postData["post_image"], postData["post_content"], postData["post_tags"], postData["post_description"], postData["post_id"]))
		except Exception as e:
